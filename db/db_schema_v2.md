# DB Schema v2.1 — SentenceMate Reader

작성: 2026-06-01 (시스템 프롬프트 v3 + 컬럼 설계 반영)
최종 업데이트: 2026-06-09 (**v2.1**: 백엔드 설계 반영 — `ai_usage`(일일 쿼터) 테이블 추가, profiles/인구통계 MVP 제외 명시. 이전 v2 검토: last_reviewed_at·updated_at 추가, 데이터 생명주기 명문화, 인덱스 주석 정정, 중복 카드 정책 보류)
관련: [../plan/backend_design_v1_26_06_09.md](../plan/backend_design_v1_26_06_09.md) — `ai_usage`의 쓰임(Gemini 프록시 쿼터 배선)
관련 문서:
...
| **sentences**                  | 인상 깊은 문장 컬렉션 (학습 단위 아님, 보관용). 데이터 모델만 현재 빌드, UI는 다음 단계         |
| **reading_sessions**           | 읽기 세션 기록 (시작/종료/CFI 위치/챕터/마지막 활동 시각)                                       |
| **ai_usage** (v2.1)            | 일일 AI 호출 쿼터 카운터 (user_id+date PK, count). Edge Function 프록시가 service_role로만 접근 |
...
> **ERD 미표기 보정(v2.1)**: 위 그림은 v2 초안이라 다음이 빠져 있음 — ① **`ai_usage`** 테이블(auth.users 1:N, `(user_id, usage_date)` PK, service_role 전용. 컬럼 SQL 참조), ② `sentences`·`user_books`의 **`updated_at`**(SQL엔 있음, moddatetime 트리거). cards에만 그려져 있으나 sentences/user_books도 동일.
...
### 5. sentence_thinking 처리 (현재 빌드)

시스템 프롬프트 v3에 `sentence_thinking` 배열이 있지만, 현재 빌드에선 **응답에 표시만 하고 DB 저장 안 함**.

근거:

- 빈도 낮음 — 대부분 응답에서 빈 배열
- 저장 자리가 모호 — 어느 카드에도 매달지 않으니 자체 행을 만들어야 함
- sentence 카드 UI가 다음 단계에서 활성화되면, 그때 "이 문장 저장" 시 sentence_thinking이 있으면 `sentences.note` 필드에 자동 통합
...
-- ============================================
-- sentences (인상 깊은 문장 컬렉션, UI는 다음 단계)
-- ============================================
...
-- ============================================
-- ai_usage (일일 AI 호출 쿼터, v2.1 — 백엔드 설계)
-- ============================================
-- Gemini 프록시(Edge Function)의 per-user 일일 쿼터 배선. 목적은 폭주 차단(비용 절감 아님).
-- 체크는 Gemini 호출 前(한도 초과면 429), 증가는 성공 後(실패는 안 셈). 한도 기본 100/일(env로 조정).
-- 상세: ../plan/backend_design_v1_26_06_09.md ⑤
...
-- 인덱스: PK (user_id, usage_date)가 "오늘 이 user의 카운트" 조회를 완전 커버 → 별도 인덱스 불필요
```
...
-- SRS 복습 (다음 단계): 복습 일정 잡힌 카드만 인덱싱 (부분 인덱스)
...
6개 public 테이블에 모두 RLS 켜고 정책 명시(books·user_books·cards·sentences·reading_sessions + v2.1 `ai_usage`). Supabase는 클라이언트가 anon key로 직접 DB 접근하므로 RLS가 보안의 마지막 안전망. `ai_usage`만 사용자 정책 없이 service_role 전용(아래 6번).
...
-- ============================================
-- 6. ai_usage — service_role 전용 (v2.1)
-- ============================================
-- RLS는 켜되 사용자 정책을 주지 않음 → anon/authenticated는 누구도 직접 SELECT/INSERT/UPDATE/DELETE 불가.
-- Edge Function이 service_role key로만 읽고 씀(쿼터 조작 차단). 클라가 카운트를 못 건드리는 게 핵심.
-- 향후 "남은 횟수" 표시가 필요하면 SELECT 정책(user_id = auth.uid())만 추가.
ALTER TABLE public.ai_usage ENABLE ROW LEVEL SECURITY;
-- (정책 없음 = service_role bypass 외 전면 차단)
...
const { data: existing } = await supabase   // maybeSingle()은 { data, error } 반환 → data 구조분해
  .from('books')
  .select('book_id')
  .eq('book_hash', hash)
  .maybeSingle();
...
해결: plan_v2 I의 결정대로 **익명 시작 + 자발적 이메일/Google OAuth 연결 유도**. 연결 후 같은 user_id 유지되어 다른 단말기 로그인 시 모든 데이터 자동 공유. 시점·UX는 현재 빌드 UI 단계 미정 항목.
...
## 시드 데이터 정책 (현재 빌드 인프라 결정만)

저작권 free 책 50~100권을 운영자가 미리 깔아두는 정책. **DB 설계 영역의 결정만 여기서 확정**, 책 선별·사용자 노출 UX·books 추가 컬럼은 다음 단계(UI)로 미룸.
...
- [ ] **시드 데이터 큐레이션 운영** (다음 단계 UI) — 실제 50권 선별, 난이도 분류, 장르 균형
- [ ] **사용자 노출 UX** (다음 단계 UI) — 자동 추가 / 정적 카탈로그 / AI 추천 시스템 등 큰 방향 결정
- [ ] **books 테이블 추가 컬럼** (UX 방향 결정 후) — difficulty/year/description/cover_url/genre/embedding 등
- [ ] **sentence_thinking 저장 정책** — 다음 단계 sentence 카드 UI 활성화 시 sentences.note에 흡수 또는 별도 컬럼 (현재는 미정)
...
- ✅ 인덱스 설계
- ✅ RLS 정책
- ✅ 저작권 free 책 시드 데이터 인프라 (책 선별·UX는 다음 단계 UI)
