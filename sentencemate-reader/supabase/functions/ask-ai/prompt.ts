// ask-ai 프롬프트 자산 — 서버 전용(클라 번들에 노출 안 됨, backend_design ① B 전략).
// SYSTEM_PROMPT 본문은 정본 system_prompt_v3_final_en.md "System Prompt Body" 블록을
// 스크립트(_gen_prompt.cjs)로 byte-fidelity 임베드한 것 — 손으로 고치지 말고 정본을 고친 뒤 재생성.
// responseSchema는 Gemini REST 호환을 위해 Type enum을 대문자(OBJECT/ARRAY/STRING)로 표기(정본 의미 동일).

export const SYSTEM_PROMPT = `You are a learning companion for a Korean reader working through an English-language novel. Specifically, you are a bilingual friend with native-level English fluency who deeply understands where Korean learners get stuck and why. You have a background in cognitive linguistics and Second Language Acquisition (SLA), so you explain English in terms of meaning and perceptual structure rather than formal labels.
...
// generationConfig.responseSchema — v3 JSON 구조 강제 (정본 JSON Response Schema 블록).
...
// user message 마커 조립 (정본 'User Message 형식'). 값 없는 줄은 출력 안 함.
// 배열 순서 계약(이 함수가 정의): prev/next는 문서 순서.
//   prev = [Previous 2, Previous 1], next = [Next 1, Next 2].
export function buildUserMessage(
  { bookInfo, prev = [], selected, next = [], userRequest } = {},
) {
  const lines = [];
  if (bookInfo && bookInfo.title) {
    const author = bookInfo.author ? " \u2014 " + bookInfo.author : "";
    lines.push("[Book Info] " + bookInfo.title + author);
  }
  if (prev[0]) lines.push("[Previous 2] " + prev[0]);
  if (prev[1]) lines.push("[Previous 1] " + prev[1]);
  lines.push("[Selected] " + selected);
  if (next[0]) lines.push("[Next 1] " + next[0]);
  if (next[1]) lines.push("[Next 2] " + next[1]);
  if (userRequest) lines.push("[User Request] " + userRequest);
  lines.push("");
  lines.push("\uc704\uc758 [Selected] \ubb38\uc7a5\uc744 \uc124\uba85\ud574\uc8fc\uc138\uc694.");
  return lines.join("\n");
}
