...
## 현재 마일스톤
→ **M5 완료 — 핵심 루프 완주(분수령).** 책→선택→AI→⊕저장→단어장이 실제로 돈다 = MVP 동작.
- 완료: M0(토대)·M1(DB)·M2(라이브러리/책추가)·M3(리더/선택)·M4(AI 4축 렌더)·M5(카드저장/단어장).
- 다음: **M6**(follow-up 멀티턴 + CFI 진척 + reading_sessions) → M7(플래시카드 + 통계) → M8(가입유도 + PWA + 출시).
