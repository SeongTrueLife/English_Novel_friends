...
앱 첫 진입
  ...
  세션 복원 + 토큰 자동 갱신(supabase-js 기본: persistSession + autoRefreshToken)
       ...
       │
       ▼ (사용자가 원할 때, 가입 유도)
  이메일/비번 연결  supabase.auth.linkIdentity(...)  → 같은 user_id 유지, 데이터 그대로 승계
```
...
| **가입 식별자** | **이메일+비번, "Confirm email" 끔** | 확인 메일 왕복 제거 = 가벼움. username-only는 비번 복구 불가라 탈락 |
| **익명→가입** | `linkIdentity`로 user_id 유지 (**모델 2**) | 익명 데이터 그대로 승계, 마이그레이션 0 |
